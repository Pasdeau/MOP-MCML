...
void ReportResult(InputStruct In_Parm, OutStruct Out_Parm) {
  ...
  /* Note: comparing against a copy of Out_Parm */
 ...
 * Modification: Added parameter summary_fp for CSV summary.
 ***********************************************************/
static void DoOneRun(short NumRuns, InputStruct *In_Ptr, FILE *summary_fp) {
  ...
  do {
    ...
    /* Reflection PD (Upper surface window) */
    if (photon.z <= epsilon &&
    ...
    /* Transmission PD (Lower surface window) */
  ...
  /* These are PD aperture metrics, printed for reference only */
  Reflectance =
      (out_parm.photonsnbrR > 0) ? (w_totR / out_parm.photonsnbrR) : 0.0L;
  Transmittance =
      (out_parm.photonsnbrT > 0) ? (w_totT / out_parm.photonsnbrT) : 0.0L;
  ...
  /* Write single run result file (including RAT) */
  ReportResult(*In_Ptr, out_parm);

  /* Key: Scale again on the current out_parm (body) so that
     out_parm.Rd / out_parm.Tt matches the RAT in the file */
  SumScaleResult(*In_Ptr, &out_parm);

  /* Append to summary.csv and print a line to terminal */
  if (summary_fp) {
    /* CSV: output,Rd,Tt */
    fprintf(summary_fp, "%s,%.10g,%.10g\n", In_Ptr->out_fname, out_parm.Rd,
            out_parm.Tt);
    fflush(summary_fp);
  }
...
/***********************************************************
 * main
 * Modify: Open summary.csv, pass file pointer to DoOneRun in loop.
 ***********************************************************/
