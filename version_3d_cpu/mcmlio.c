...
void ShowVersion(const char *version) {
  ...
  CenterStr(COLWIDTH,
            "MCML - Monte Carlo Simulation of Multi-layered Turbid Media", str);
  puts(str);
  puts("");
  ...
  CenterStr(COLWIDTH, "Laser Biology Research Laboratory - Box 17", str);
  puts(str);
...
void ReadDzDr(FILE *File_Ptr, InputStruct *In_Ptr) {
  ...
  int count = sscanf(buf, "%lf%lf%lf%lf", &In_Ptr->dz, &In_Ptr->dr, &In_Ptr->dx,
                     &In_Ptr->dy);
  if (count < 4) {
    In_Ptr->dx = In_Ptr->dr;
    In_Ptr->dy = In_Ptr->dr;
  ...
  if (In_Ptr->dx <= 0)
    nrerror("Nonpositive dx.\n");
  if (In_Ptr->dy <= 0)
    nrerror("Nonpositive dy.\n");
}
...
void ReadNzNrNa(FILE *File_Ptr, InputStruct *In_Ptr) {
  ...
  strcpy(buf, FindDataLine(File_Ptr));
  if (buf[0] == '\0')
    nrerror("Reading number of dz, dr, da, dx, dy's.\n");
  int count = sscanf(buf, "%hd%hd%hd%hd%hd", &In_Ptr->nz, &In_Ptr->nr,
                     &In_Ptr->na, &In_Ptr->nx, &In_Ptr->ny);
  if (count < 5) {
    In_Ptr->nx = 100;
    In_Ptr->ny = 100;
  }
...
void ReadPD(FILE *File_Ptr, InputStruct *In_Ptr) {
  ...
  if (In_Ptr->PD_Rl <= 0)
    nrerror("Nonpositive PD_Rl.\n");
  if (In_Ptr->PD_Tl <= 0)
    nrerror("Nonpositive PD_Tl.\n");
}
...
void InitOutputData(InputStruct In_Parm, OutStruct *Out_Ptr) {
  ...
  Out_Ptr->OP_3D = (double *)malloc((long)nx * ny * nz * sizeof(double));
  if (!Out_Ptr->OP_3D)
    nrerror("allocation failure for OP_3D");
  memset(Out_Ptr->OP_3D, 0, (long)nx * ny * nz * sizeof(double));
...
static void WriteInParm(FILE *file, InputStruct In_Parm) {
  ...
  fprintf(file, "%hd\t%hd\t%hd\t%hd\t%hd\t# No. of dz, dr, da, dx, dy.\n\n",
...
static void WriteRd_ra(FILE *file, short Nr, short Na, OutStruct Out_Parm) {
  short ir, ia;
  fprintf(file, "%s\n%s\n%s\n%s\n%s\n%s\n", "# Rd[r][angle]. [1/(cm2sr)].",
          "# Rd[0][0], [0][1],..[0][na-1]", "# Rd[1][0], [1][1],..[1][na-1]",
...
static void WriteTt_ra(FILE *file, short Nr, short Na, OutStruct Out_Parm) {
  short ir, ia;
  fprintf(file, "%s\n%s\n%s\n%s\n%s\n%s\n", "# Tt[r][angle]. [1/(cm2sr)].",
          "# Tt[0][0], [0][1],..[0][na-1]", "# Tt[1][0], [1][1],..[1][na-1]",
...
static void WriteA_rz(FILE *file, short Nr, short Nz, OutStruct Out_Parm) {
  short iz, ir;
  fprintf(file, "%s\n%s\n%s\n%s\n%s\n%s\n", "# A[r][z]. [1/cm3]",
          "# A[0][0], [0][1],..[0][nz-1]", "# A[1][0], [1][1],..[1][nz-1]",
...
static void WriteOP_3D(FILE *file, short nx, short ny, short nz,
                       OutStruct Out_Parm, char fformat) {
  long ix, iy, iz;
  fprintf(file, "%s\n%s\n%s\n%s\n%s\n%s\n", "# OP_3D[x][y][z]. [1/cm3]",
  ...
  if (toupper(fformat) == 'A') {
    ...
    fwrite(Out_Parm.OP_3D, sizeof(double), nx * ny * nz, file);
    fprintf(file, "\n");
  }
}
