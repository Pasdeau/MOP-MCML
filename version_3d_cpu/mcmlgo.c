...
void Drop(InputStruct *In_Ptr, PhotonStruct *Photon_Ptr, OutStruct *Out_Ptr,
          int *cpt, double *sOut, short *ixOut, short *iyOut, short *izOut) {
  (void)In_Ptr;
  ...
  /* Compute 3D indices
     The user requested specifically that the physical coordinate x,y Maps
   directly to ix, iy where 0 is nx/2. Thus x=-0.2 maps to an offset index. */
  ...
  /* Record some step info into tracing arrays */
  int idx = *cpt;
  if (idx < 60000) {
    ixOut[idx] = ix;
    iyOut[idx] = iy;
 ...
 *  Record R and T.
 ****/
void RecordR(double Refl, InputStruct *In_Ptr, PhotonStruct *Photon_Ptr,
             OutStruct *Out_Ptr) {
...
void HopDropSpinInTissue(InputStruct *In_Ptr, PhotonStruct *Photon_Ptr,
                         OutStruct *Out_Ptr, int *cpt, double *sOut,
                         short *ixOut, short *iyOut, short *izOut) {
  ...
  if (HitBoundary(Photon_Ptr, In_Ptr, Out_Ptr)) {
    ...
    Hop(Photon_Ptr);
    Drop(In_Ptr, Photon_Ptr, Out_Ptr, cpt, sOut, ixOut, iyOut, izOut);
...
void HopDropSpin(InputStruct *In_Ptr, PhotonStruct *Photon_Ptr,
                 OutStruct *Out_Ptr, int *cpt, double *sOut, short *ixOut,
                 short *iyOut, short *izOut) {
  short layer = Photon_Ptr->layer;
  ...
  if ((In_Ptr->layerspecs[layer].mua == 0.0) &&
    ...
    HopDropSpinInTissue(In_Ptr, Photon_Ptr, Out_Ptr, cpt, sOut, ixOut, iyOut,
                        izOut);
